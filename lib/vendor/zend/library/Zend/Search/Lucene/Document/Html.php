...
class Zend_Search_Lucene_Document_Html extends Zend_Search_Lucene_Document
{
  ...
  // CUSTOM
  public function highlightN($words)
  {
    $analyzer = Zend_Search_Lucene_Analysis_Analyzer::getDefault();
    $hl = call_user_func_array('array_merge', array_map(
      array($analyzer, 'tokenize'),
      (array) $words, array_fill(0, count((array) $words), 'utf-8')
    ));
    ...
    $hl_flipped = array();
    foreach ($hl as $id => $token) {
      $hl_flipped[$token->getTermText()] = $id;
    }
    ...
    $matchedNodes = $xpath->query("/html/body");
    foreach ($matchedNodes as $matchedNode) {
      $this->_highlightNNodeRecursive($matchedNode, $hl_flipped);
    }
  }


  protected function _highlightNNodeRecursive(DOMNode $contextNode, $hl_flipped)
  {
    $textNodes = array();
    ...
    foreach ($contextNode->childNodes as $childNode) {
      if ($childNode->nodeType == XML_TEXT_NODE) {
        ...
        // Process node if it's not a script node
        if ($childNode->nodeName != 'script') {
          $this->_highlightNNodeRecursive($childNode, $hl_flipped);
        }
      }
    }
    ...
    foreach ($textNodes as $node) {
      ...
      while (($token = $analyzer->nextToken()) !== null) {
        ...
        if (isset($hl_flipped[$token->getTermText()])) {
          $tokens[] = $token;
        }
      }
      ...
      foreach (array_reverse($tokens) as $token) {
        ...
        $hl_node = $this->_doc->createElement('mark', $matchedWordNode->nodeValue);
        $hl_node->setAttribute('class', 'highlight');

        $node->parentNode->insertBefore($hl_node, $matchedWordNode);
        $node->parentNode->removeChild($matchedWordNode);
      }
    }
  }
